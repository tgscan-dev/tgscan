...
public class RoomDoc {
    ...
    public static RoomDoc fromEntity(Room room) {
        ...
        var newLang = Arrays.stream(
                        Optional.ofNullable(room.getLang()).orElse("").split(","))
                .map(String::trim)
        ...
        var newTags = Arrays.stream(tags)
                .map(String::trim)
                .map(String::toLowerCase)
                .distinct()
                .collect(Collectors.toList());
        roomDoc.setTags(newTags);
        var newCategory = Arrays.stream(
                        Optional.ofNullable(room.getCategory()).orElse("").split(","))
                .map(String::trim)
                .map(String::toLowerCase)
                .distinct()
                .collect(Collectors.toList());
